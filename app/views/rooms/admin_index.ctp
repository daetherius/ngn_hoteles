...
echo
	$this->element('adminhdr',array(
		'title'=>'Habitaciones - '.$nombre_hotel,
		'links'=>array(
	...
	echo
		...
		$html->tag('table',null,array('class'=>'datagrid')),
			$html->tableHeaders(array(
				...
				$paginator->sort('Disponibilidad', 'disponibilidad'),
			...
			foreach($items as $it){
				...
				echo $html->tableCells(array(array(
					...
					$it[$_m[0]]['nombre'],
					$it[$_m[0]]['disponibilidad'],
					$util->toggle($it[$_m[0]]['activo'],$id),
					$util->fdate('s',$it[$_m[0]]['created']),
					array(
						$html->link('Tarifas',array('controller'=>'rates','action'=>'index','admin'=>1,'room_id'=>$id)).
