		...
		$html->tag('table',null,array('class'=>'datagrid')),
			...
			foreach($items as $it){
				...
				echo $html->tableCells(array(array(
					...
					array(
						$html->link('Habitaciones',array('controller'=>'rooms','action'=>'index','admin'=>1,'hotel_id'=>$id)).
						$html->link('Blackouts',array('controller'=>'blackouts','action'=>'index','admin'=>1,'hotel_id'=>$id)).
						($hasimgs ? $html->link('Fotos',array('action'=>'images','admin'=>1,$id)):'').
						$html->link('Editar',array('action'=>'editar','admin'=>1,$id)).
						($isMaster ? $html->link('Eliminar',array('action'=>'eliminar','admin'=>1,$id),null,'¿Seguro que quiere eliminar este elemento?') : '')
					,array('class'=>'actions'))
				)),array('class'=>'odd'));
			}
